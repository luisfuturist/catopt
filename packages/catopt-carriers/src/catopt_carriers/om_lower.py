...
After eqsat with ``OM_LAWS`` (see :mod:`catopt_carriers.om`), chunked attention
...
:class:`~catopt_torch.torch_bridge.IRModule` evaluates it serially by passing
...
:mod:`catopt_carriers.scan_lower` for the affine monoid:
...
def build_om_plan(root: Any) -> dict | None:
    ...
    * ``"levels"`` / ``"level_gather"`` — as in
      :func:`catopt_carriers.scan_lower.build_scan_plan`: ``level_gather[k]`` is
...
class StreamingOMModule(torch.nn.Module):
    ...
    Ports layer: a :class:`catopt_core.ports.PlannedExecutor` (``forward`` +
    ``_plan``, with ``eval_mod`` as the serial fallback) but
    deliberately not a :class:`~catopt_core.ports.BatchedExecutor` — its
