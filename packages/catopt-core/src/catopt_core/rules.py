"""Compatibility shim — the rewrite surface moved to :mod:`catopt_core.laws`.

Historically this module mixed three different things: the equational
law definitions (now :mod:`catopt_core.laws.tensor` and
:mod:`catopt_core.laws.scan`), the side-condition check helpers (same
modules as their laws), and the non-local whole-graph passes
(:mod:`catopt_core.laws.pairing`).  Everything previously importable from
``catopt_core.rules`` — public rule objects, rule collections,
