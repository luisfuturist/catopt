...
def main() -> None:
    ...
    print(f"{'case':<24} {'equiv':>10} {'inductor ms':>12}"
          f" {'catopt ms':>10} {'speedup':>9}")
    print("-" * 70)

    cases = [
        ("MatrixChain b=4096", MatrixChain(128, 64, 32, 8),
         torch.randn(4096, 128)),
        ("ParallelLinear b=4096", ParallelLinear(512, 2),
         torch.randn(4096, 512)),
        ("DeepParallel b=4096", DeepParallel(512, 512, 512),
         torch.randn(4096, 512)),
        ("SwiGLU b=4096", SwiGLU(512, 4), torch.randn(4096, 512)),
        ("SwiGLU b=128", SwiGLU(512, 4), torch.randn(128, 512)),
        ("Attention b=64 T=256", AttentionBlock(512, 8),
    ...
    for name, model, x in cases:
        ...
        print(f"{name:<24} {d:>10.2e} {r_o.mean_ms:>12.4f}"
