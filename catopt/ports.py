...
------------------------------------
``catopt.ir`` terms, ``catopt.egraph`` (union-find, e-matching,
equality saturation, extraction, certificates), ``catopt.laws``
(equational rewrites), ``catopt.typing`` (shape inference),
``catopt.cost`` (pricing).  These know about terms and e-classes —
...
class OpRegistry(Protocol):
    ...
    ``catopt.ops.OpTable`` is the canonical (and only) implementation —
...
class ShapeRule(Protocol):
    ...
    Domain port — installed in ``catopt.typing._SHAPE_RULES`` and
