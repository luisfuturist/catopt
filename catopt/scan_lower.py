...
class BatchedScanModule(torch.nn.Module):
    ...
    def capture_cuda_graph(
        ...
        buffers and replays the graph — one launch total instead of
        ~4 per tree level.  Returns ``self`` for chaining; a no-op when
        the module is not batched or CUDA is unavailable.

        Caveats (standard CUDA-graph rules): inputs must be CUDA
        tensors; calls with a different shape/dtype/device fall back to
        the eager path; parameters are baked in by pointer — in-place
        updates are replayed, replacing a parameter is not; the
        returned tensor is the static output buffer, overwritten by the
