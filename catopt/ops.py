...
mutable state: ``catopt.torch_bridge._IR_TO_TORCH`` was seeded with the
core bindings and every carrier module (``catopt.trace``,
``catopt.xcarrier``, ``catopt.om``, ``catopt.act_eps``) mutated it at
...
* :meth:`OpTable.core` — the base table: the core torch bindings plus
  the ambient shape rules (``catopt.typing._SHAPE_RULES``) and the
  attr schema (``catopt.attrs.ATTR_SCHEMA``).
...
* :mod:`catopt.trace` — ``trace`` ``bdiag`` ``parl`` ``eye`` ``cswap``
  ``inv``
* :mod:`catopt.xcarrier` — ``affd_a`` ``affd_b`` ``aff_A`` ``aff_b``
  ``om_elem_affd`` ``om_elem_aff`` ``omd`` ``omd_elem`` ``omd_compose``
  ``omd_apply`` ``omd_applym``
* :mod:`catopt.om` — ``cmask`` ``fill`` ``attnbias``
* :mod:`catopt.act_eps` — ``aquant`` ``adequant``
...
class OpTable:
    ...
    def core(cls) -> OpTable:
        ...
        from catopt.torch_bridge import _CORE_TORCH_BINDINGS
    ...
    def full(cls) -> OpTable:
        ...
        ``catopt.torch_bridge._IR_TO_TORCH`` dict itself: post-hoc
