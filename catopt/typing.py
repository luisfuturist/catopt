...
This layer is deliberately separate from pricing (``catopt.cost``):
...
inferred structurally from ``catopt.ir`` terms and their attrs.
...
def _infer_op_shape(op: Op, memo: dict | None = None):
    ...
    # like catopt.trace's ``eye``/``cswap``): their shapes are fully
...
def _bdiag_shape(op: Op, shapes: list) -> tuple | str | None:
    ...
    keeping feedback wires first (see catopt.trace)."""
    a, b = shapes[0], shapes[1]
    if (
        isinstance(a, tuple)
