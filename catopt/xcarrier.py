...
def _om_elem_affd(s: torch.Tensor, a: torch.Tensor, b: torch.Tensor,
                  h: torch.Tensor):
...
def _check_matmul_applyd_vec(bound: dict) -> bool:
    ...
    if not all(_concrete(s) for s in (W, a, b, h)):
        return False
    return (len(W) == 2 and len(a) == 1 and a == b == h
            and W[-1] == a[-1])
...
def _check_matmul_apply(bound: dict) -> bool:
    ...
    if not all(_concrete(s) for s in (W, A, c, h)):
        return False
    return (len(W) == 2 and len(A) == 2 and A[0] == A[1] == W[-1]
            and len(c) == 1 and c == h and c[-1] == A[-1])
...
def _check_linear_applyd(bound: dict) -> bool:
    ...
    if not all(_concrete(s) for s in (W, a, b, h)):
        return False
    return (len(W) == 2 and len(a) >= 1 and a == b
            and len(h) == 1 and h[-1] == a[-1] == W[-1])
...
def _check_linear_apply(bound: dict) -> bool:
    ...
    if not all(_concrete(s) for s in (W, A, c, h)):
        return False
    if not (len(W) == 2 and len(A) >= 2 and A[-1] == A[-2] == W[-1]
            and len(h) == 1 and h[-1] == A[-1] and len(c) >= 1):
        return False
...
def _scalar_or_broadcast(bound: dict, rkey: str, tshape) -> bool:
    ...
    return rs is not None and _broadcast_ok(rs, tshape) and (
...
def _check_scale_apply(bound: dict) -> bool:
    ...
    if rs == A[:-1]:
        return True
    return (len(rs) == len(A) - 1 and rs[-1] == 1
...
def _check_add_apply(bound: dict) -> bool:
    ...
    if not all(_concrete(s) for s in (f, g, h)):
        return False
    return (f == g and len(f) >= 2 and f[-1] == f[-2]
            and len(h) == 1 and h[-1] == f[-1])
...
def _check_chunk_apply(bound: dict) -> tuple[bool, int | None]:
    ...
    A, c, h = (_shape(bound.get(k)) for k in ("A", "c", "h"))
    D = bound.get("$attr:D")
    if not (all(_concrete(s) for s in (A, c, h)) and isinstance(D, int)):
        return False, None
    if not (len(c) >= 1 and len(A) == len(c) + 1 and A[:-1] == c
...
def _check_split_sizes_int(bound: dict) -> bool:
    ...
    return (isinstance(sz, (list, tuple))
            and all(isinstance(x, int) for x in sz))
...
def _check_split_apply(bound: dict) -> bool:
    return _check_chunk_apply_ok(bound) and _check_split_sizes_int(bound)
...
def _check_om_elem_affd(bound: dict) -> bool:
    ...
    if not all(_concrete(x) for x in (s, a, b, h)):
        return False
    if not (len(s) >= 2 and len(a) >= 2 and a == b
            and len(h) == 1):
        return False
...
def _check_omd_pair(bound: dict) -> bool:
    ...
    ok1 = _check_om_elem_affd(
        {"s": bound.get("s1"), "a": bound.get("a1"),
         "b": bound.get("b1"), "h": bound.get("h")})
    ok2 = _check_om_elem_affd(
        {"s": bound.get("s2"), "a": bound.get("a2"),
         "b": bound.get("b2"), "h": bound.get("h")})
    if not (ok1 and ok2):
        return False
    s1, s2 = _shape(bound.get("s1")), _shape(bound.get("s2"))
    a1, a2 = _shape(bound.get("a1")), _shape(bound.get("a2"))
    return (all(s1[i] == s2[i] for i in range(len(s1) - 1))
            and all(a1[i] == a2[i] for i in range(len(a1))
                    if i != len(a1) - 2))
...
def _check_omd_split(bound: dict) -> bool:
    ...
    sd, ad, bd = (bound.get("$attr:SD"), bound.get("$attr:AD"),
                  bound.get("$attr:BD"))
    s1, s2 = _shape(bound.get("s1")), _shape(bound.get("s2"))
    a1, a2 = _shape(bound.get("a1")), _shape(bound.get("a2"))
    ...
    if not all(s1[i] == s2[i] for i in range(len(s1) - 1)):
        return False
    if not (all(a1[i] == a2[i] for i in range(len(a1))
                if i != len(a1) - 2)
            and s1[-1] == a1[-2] and s2[-1] == a2[-2]):
        return False
    return (_broadcast_ok(s1[:-2], a1[:-2])
            and _broadcast_ok(s2[:-2], a2[:-2]))
...
XC_MATMUL_APPLYD_VEC = R(
    ...
    Op.make("matmul", "W", Op.make("applyd",
                                   Op.make("aff_diag", "a", "b"), "h")),
    Op.make("apply",
            Op.make("aff",
                    Op.make("mul", "W", "a"),
                    Op.make("matmul", "W", "b")),
...
def _scale_apply(name, r_first):
    ...
    lhs = Op.make("mul", "r", base) if r_first else Op.make(
        "mul", base, "r")
    return R(
        name, lhs,
        Op.make("apply",
...
def _chunk_applyd(name, op, extra_check):
    lhs = (Op.make("chunk", Op.make("applyd",
                                    Op.make("aff_diag", "a", "b"),
                                    "h"),
                   chunks="N", dim="D", index="I")
           if op == "chunk" else
           Op.make("split", Op.make("applyd",
                                    Op.make("aff_diag", "a", "b"),
                                    "h"),
                   sizes="SZ", dim="D", index="I"))
    inner = (Op.make("chunk", "a", chunks="N", dim="D", index="I"),
    ...
    return R(
        name, lhs,
        Op.make("applyd",
                Op.make("aff_diag", inner[0], inner[1]), "h"),
...
XC_CHUNK_APPLYD = _chunk_applyd("xc_chunk_applyd", "chunk",
                                _check_chunk_applyd_ok)
XC_SPLIT_APPLYD = _chunk_applyd("xc_split_applyd", "split",
                                _check_split_applyd)
...
def _chunk_apply(name, op, check):
    ...
    return R(
        ...
        law=f"{op} distributes over a dense-affine application — sound "
            ...
            "sliced).",
        check=check, derive=_derive_chunk_apply)


XC_CHUNK_APPLY = _chunk_apply("xc_chunk_apply", "chunk",
                              _check_chunk_apply_ok)
XC_SPLIT_APPLY = _chunk_apply("xc_split_apply", "split",
                              _check_split_apply)
...
XC_OMD_PAIR_LIFT = R(
    ...
    Op.make("om_apply",
            Op.make("om_compose",
                    Op.make("om_elem", "s1",
                            Op.make("applyd",
                                    Op.make("aff_diag", "a1", "b1"),
                                    "h")),
                    Op.make("om_elem", "s2",
                            Op.make("applyd",
                                    Op.make("aff_diag", "a2", "b2"),
                                    "h")))),
    Op.make("omd_apply",
            Op.make("omd_compose",
...
XC_LAWS: list[Rewrite] = [
    XC_MATMUL_APPLYD_ROWS, XC_MATMUL_APPLYD_ROWS_REV,
    XC_MATMUL_APPLYD_VEC, XC_MATMUL_APPLYD_VEC_REV,
    XC_MATMUL_APPLY, XC_MATMUL_APPLY_REV,
    XC_LINEAR_APPLYD, XC_LINEAR_APPLYD_REV,
    XC_LINEAR_APPLY, XC_LINEAR_APPLY_REV,
    XC_SCALE_APPLYD, XC_SCALE_APPLYD_PRE,
    ...
    XC_OMD_LIFT_DENSE, XC_OMD_UNLIFT_DENSE,
    XC_OMD_PAIR_LIFT,
    XC_OMD_SPLIT, XC_OMD_SPLIT_ARG1,
]
...
def _offer_witness(eg: EGraph, cid: int, offered: Any,
                   offered_eid: int, provenance: str):
    ...
    return Rewrite(
        ...
        lhs=src, rhs=offered,
        law=("pointwise witness for a non-local offer: stack of "
...
def _map_out_shape(f_shape, h_shape, kind: str):
    ...
    if (isinstance(f_shape, tuple) and len(f_shape) >= 2
            and f_shape[-1] == f_shape[-2]):
        return f_shape[:-1]
    return None


def _gather_stack(eg: EGraph, apply_op: str, proj_a: str, proj_b: str,
                  pack_op: str, kind: str, witness: bool,
            ...
            for h_eid in sorted(common):
                ...
                for o in opts:
                    ...
                    for f in sorted(o[h_eid]):
                        ...
                        if (vs is None or not _concrete(vs)
                                or vs in m):
                            continue
                ...
                for S in sorted(shared):
                    ...
                    sa = eg.add_enode(
                        "stack", tuple(eg.add_enode(proj_a, (f,))
                                       for f in f_ids), attrs)
                    sb = eg.add_enode(
                        "stack", tuple(eg.add_enode(proj_b, (f,))
                                       for f in f_ids), attrs)
                    fmap = eg.add_enode(pack_op, (sa, sb))
                    out = eg.add_enode(apply_op, (fmap, h_eid))
                    offered_term = eg.any_term(out)
                    w = (_offer_witness(eg, c, offered_term, out,
                                        provenance)
                         if witness and offered_term is not None
                         else None)
                    eg.union(c, out, witness=w,
                             note=(f"{provenance}: stack of "
                                   f"{len(kids)} {apply_op} members "
                                   "over shared h"))
                    offers.append({"stack_eid": c, "h_eid": h_eid,
                                   "out_eid": out,
                                   "term": offered_term})
    return offers


def gather_applyd_stack(eg: EGraph, *, witness: bool = True,
                        provenance: str = "xc_stack_diag") -> list:
    ...
    return _gather_stack(eg, "applyd", "affd_a", "affd_b", "aff_diag",
                         "diag", witness, provenance)


def gather_apply_stack(eg: EGraph, *, witness: bool = True,
                       provenance: str = "xc_stack_dense") -> list:
    """Dense analog: ``apply(aff(stack aff_A f_i, stack aff_b f_i), h)``."""
    return _gather_stack(eg, "apply", "aff_A", "aff_b", "aff",
                         "dense", witness, provenance)
        ...
        for fn in eg.get_class(fcid).nodes:
            if fn.op == pack and len(fn.children) == 2:
                out.setdefault(hcid, []).append(
                    (kind, eg.find(fn.children[0]),
                     eg.find(fn.children[1])))
    return out


def _omd_convert(eg: EGraph, cid: int, h_eid: int, kind: str,
                 memo: dict, stack: frozenset):
    ...
    stack = stack | {cid}
    res = None
    for n in sorted(eg.get_class(cid).nodes,
        ...
        if n.op == "om_elem" and len(n.children) == 2:
            ...
            f = _omd_convert(eg, n.children[0], h_eid, kind, memo,
                             stack)
            g = _omd_convert(eg, n.children[1], h_eid, kind, memo,
                             stack)
...
def omd_tree_lift(eg: EGraph, *, witness: bool = True,
                  provenance: str = "omd_lift") -> list[dict]:
    ...
    for cid in list(eg._classes.keys()):
        ...
        for node in list(ec.nodes):
            ...
            for h_eid in sorted(h_cands):
                for kind, apply_op in (("diag", "omd_apply"),
                                       ("dense", "omd_applym")):
                    root = _omd_convert(eg, carrier, h_eid, kind,
                                        memo, frozenset())
                    if root is None:
                        continue
                    out = eg.add_enode(apply_op, (root, h_eid))
                    offered = eg.any_term(out)
                    w = (_offer_witness(eg, c, offered, out,
                                        provenance)
                         if witness and offered is not None else None)
                    eg.union(c, out, witness=w,
                             note=(f"{provenance}: om tree over "
                                   f"{kind}-affine values, shared h"))
                    offers.append({"om_apply_eid": c, "h_eid": h_eid,
                                   "out_eid": out, "term": offered,
                                   "kind": kind})
