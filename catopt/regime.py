...
def footprint_cost(term: Any, memo: Optional[dict] = None) -> float:
    ...
    if isinstance(term, (Var, Param, Const)):
        ...
        if shape == _INVALID:
            ...
            out = sum(footprint_cost(ch, memo) for ch in term.args) + own
    memo[key] = out
    return out
...
class ExecutorSpec:
    ...
    name: str
    lower: Callable[[IR, Optional[dict]], nn.Module]
    accepts: Callable[[Any], bool]
    engaged: Callable[[nn.Module], bool]
    carrier: Optional[Tuple[frozenset, frozenset, frozenset]] = None
    blurb: str = ""
...
def default_regimes() -> List[Regime]:
    ...
    * ``bounded_memory`` — min materialised elements, streaming fold.
    ...
    return [
        ...
        Regime("prefill", cost_fn=roofline_cost, executor="om_batched",
               note="roofline-bound; batched OM tree"),
        Regime("bounded_memory", cost_fn=footprint_cost,
               executor="om_streaming",
...
def _has_carrier(term: Any, carrier: Tuple[frozenset, frozenset,
                                           frozenset]) -> bool:
...
def architecture_label(term: Any) -> str:
    ...
    if is_scan_apply_term(term):
        ...
        if plan is not None:
            sched = (f"{len(plan['leaves'])} leaves / "
                     f"{len(plan['levels'])} batched levels"
                     if n_comp else
                     f"{len(plan['leaves'])} leaves / sequential apply "
                     "chain")
        else:
            sched = "unplannable"
    ...
    if is_om_apply_term(term):
        ...
        sched = (f"{len(plan['leaves'])} blocks / "
                 f"{len(plan['levels'])} batched levels"
...
def _force_carrier(eg: EGraph, root_eid: int, cost_fn: Callable,
                   carrier: Tuple[frozenset, frozenset, frozenset],
                   accepts: Optional[Callable] = None) -> Optional[Any]:
    ...
    for cid in list(eg._classes):
        ...
        if ec is None:
            continue
        inner = sorted((n for n in ec.nodes if n.op in inner_ops),
                       key=repr)
        roots = sorted((n for n in ec.nodes if n.op in root_ops),
                       key=repr)
...
class RegimeChoice:
    ...
    cost: Optional[float]             # dag_cost(term, cost_fn)
    cost_best: Optional[float]        # dag_cost(cost_term, cost_fn)
    ...
    rank: Optional[int] = None        # rank among alternatives by cost
    engaged: Optional[bool] = None    # module probe; set at build() time
    note: str = ""
...
class RegimeFrontier:
    ...
    def certificate(self, name: str):
        ...
        return self.eg.certificate(self.src_term, self.choices[name].term,
                                   root_eid=self.root_eid)

    # -- building ----------------------------------------------------
    def build(self, param_values: Optional[dict] = None, *,
              default: Optional[str] = None) -> "RegimeDispatch":
    ...
    def report(self) -> str:
        ...
        for name, ch in self.choices.items():
            ...
            if ch.engaged is not None:
                flags.append("engaged" if ch.engaged
...
def regime_frontier(eg: EGraph, root_eid: int,
                    regimes: Any = None, *,
                    ir: Optional[IR] = None,
    ...
    for spec in regime_list:
        ...
        if cost_term is None:
            choices[spec.name] = RegimeChoice(
                regime=spec.name, executor=spec.executor, term=None,
                cost_term=None, cost=None, cost_best=None, depth=0,
                forced=False, native=False, carrier_present=False,
                degraded=True, signature=("none",),
                label="extraction failed",
                note="extract_fn/cost_fn returned no member")
            continue

        # 2. resolve executor
        ex_name = (spec.executor if spec.executor != "auto"
        ...
        if not native and spec.prefer_executor and ex.carrier is not None:
            t = _force_carrier(eg, root_eid, spec.cost_fn or flops_cost,
                               ex.carrier, accepts=ex.accepts)
            if t is not None:
                served, forced = t, True
                native = ex.accepts(t)

        carrier_present = (ex.carrier is not None
                           and _has_carrier(served, ex.carrier))
        degraded = (ex.carrier is not None and not native
        ...
        note = spec.note
        if degraded:
            note = (f"{ex_name} carrier unreachable from this e-class; "
                    "executor runs its serial fallback"
                    + (f" ({note})" if note else ""))
        elif forced and not native:
            note = ("carrier reachable only nested below the term root; "
                    "executor's specialised schedule cannot fire"
                    + (f" ({note})" if note else ""))
        elif forced and cost is not None and cost_best is not None:
            note = (f"preferred form forced; premium "
                    f"{cost - cost_best:+.4g} under this cost model"
                    + (f" ({note})" if note else ""))

        choices[spec.name] = RegimeChoice(
            regime=spec.name, executor=ex_name, term=served,
            cost_term=cost_term, cost=cost, cost_best=cost_best,
            depth=_term_depth(served), forced=forced, native=native,
            carrier_present=carrier_present, degraded=degraded,
            signature=architecture_signature(served),
            label=architecture_label(served),
            alternatives=alternatives, rank=rank, note=note)

    return RegimeFrontier(eg=eg, root_eid=root_eid, src_term=src_term,
                          ir=ir, regimes=regime_list, choices=choices)
...
class RegimeDispatch(nn.Module):
    ...
    def __init__(self, frontier: RegimeFrontier,
        ...
        for name, ch in frontier.choices.items():
            ...
            ir_i = IR(root=ch.term, inputs=ir.inputs,
            ...
            self._meta[name] = {"term": ch.term, "module": mod,
                                "choice": ch}
        ...
        self.verification: Optional[Dict[str, dict]] = None
        self.saturation_stats: Optional[dict] = None

    # -- one set of weights ------------------------------------------
    @staticmethod
    def _param_map_of(mod: nn.Module) -> Optional[dict]:
        ...
        if pm is None:
            ...
            pm = getattr(inner, "_param_map", None) if inner is not None \
                else None
        return pm
    ...
    def _share_params(self) -> None:
        ...
        for mod in self.forms.values():
            ...
            if pm is None:
                continue
            host = mod if getattr(mod, "_param_map", None) is pm \
                else mod.eval_mod
            for pname in pm:
    ...
    def set_regime(self, name: str) -> None:
        if name not in self._meta:
            raise KeyError(f"unknown regime {name!r}; available: "
                           f"{sorted(self._meta)}")
        self._regime = name
    ...
    def forward(self, *xs: torch.Tensor,
                regime: Optional[str] = None) -> torch.Tensor:
    ...
    def verify(self, reference: Any, *xs: torch.Tensor,
               atol: float = 1e-9) -> Dict[str, dict]:
        ...
        self.verification = {n: {"max_abs_diff": d, "ok": d <= atol}
    ...
    def extra_repr(self) -> str:
        return (f"regime={self._regime!r}, "
                f"forms={list(self._meta)}")
