...
from catopt.xcarrier import (gather_applyd_stack, gather_apply_stack,
                             omd_tree_lift, XC_LAWS,
                             _elem_affine_options)
from catopt.omd_lower import to_batched_omd_module, is_omd_apply_term
from catopt.cost import flops_cost, _shape_of
...
class ScanAttnMH(_CausalScan):
    ...
    def forward(self, x):
        T = x.shape[0]
        v = self.wv(self.scan(x)).view(T, self.nh, self.hd) \
            .transpose(0, 1)
        q = self.wq(x).view(T, self.nh, self.hd).transpose(0, 1)
...
def bounded_build_xc(m, x, max_iter=4, xc_iter=4, max_nodes=300_000,
    ...
    stats = eg.run(default_rules(), root, max_iterations=max_iter,
                   max_nodes=max_nodes)
    ...
    def _lifts(trace=True):
        ...
        if trace:
            try:
                ...
                crashed = True
        return (tl + gather_applyd_stack(eg) + gather_apply_stack(eg)
                + omd_tree_lift(eg), crashed)
...
def extract_omd_term(eg, root):
    ...
    rc = eg.find(root)
    nodes = [n for n in eg.get_class(rc).nodes
             if n.op in OMD_ROOT_OPS]
    if not nodes:
        return None, 0
...
def carrier_analysis(eg, root):
    ...
    info = {
        ...
        "om_apply_any": 0, "omd_any": 0,
        "omd_at_root": sum(1 for n in eg.get_class(rc).nodes
                           if n.op in OMD_ROOT_OPS),
        "elems": [],
    }
    for cid in list(eg._classes):
        ...
        for n in ec.nodes:
            ...
            if n.op == "om_elem" and len(n.children) == 2:
                ...
                vops = sorted({nn.op for nn in
                               eg.get_class(vcid).nodes})
                ...
                info["elems"].append({
                    "class": c, "v_ops": vops,
                    "aff_h_eids": sorted(aff), "map_shape": mapshape})
    return info
...
def gap_report(m, x, tag, note=""):
    ...
    print(f"  build {time.perf_counter()-t0:.1f}s enodes={eg.n_enodes} "
          f"lifts={stats.get('nonlocal_lifts')} "
          f"trace_crash={stats.get('trace_lift_crashed')}", flush=True)
    info = carrier_analysis(eg, root)
    print(f"  root-class ops: {info['root_ops']}", flush=True)
    print(f"  om_apply enodes anywhere: {info['om_apply_any']}   "
          f"omd enodes anywhere: {info['omd_any']}   "
          f"omd at root: {info['omd_at_root']}", flush=True)
    for e in info["elems"][:8]:
        print(f"    om_elem@{e['class']}: value-class ops={e['v_ops']} "
              f"affine-opts(h_eids)={e['aff_h_eids']} "
              f"map={e['map_shape']}", flush=True)
    if len(info["elems"]) > 8:
        print(f"    … {len(info['elems']) - 8} more om_elem leaves",
              flush=True)
...
def run_mqa(args, devices):
    ...
    for T in sizes:
        ...
        meta[T] = {
            "build_s": build_s, "enodes": eg.n_enodes, "n_omd": n_omd,
        ...
        print(f"  omd census (tree): {meta[T]['census_omd']}", flush=True)
        ...
        for dev in devices:
            ...
            for name, term in terms.items():
                ...
                try:
                    mods[name] = ir_to_torch_module(
                        IR(root=term, inputs=ir.inputs,
                           params=ir.params), src).to(dev)
                except Exception as e:  # noqa: BLE001
                    print(f"    {name}: lower FAILED {e}", flush=True)
            # the batched executor on the SAME omd term
            try:
                bomd = to_batched_omd_module(
                    IR(root=omd_term, inputs=ir.inputs,
                       params=ir.params), src).to(dev)
                print(f"    omd-batched: is_batched={bomd.is_batched} "
                      f"map_mode={bomd.map_mode}", flush=True)
            ...
            for name, mod in mods.items():
                try:
                    ...
                    rows[f"{name}-IR (generic)"] = (
                        tms, err, n, meta[T]["ops"].get(name))
                    print(f"    {name}: {tms:.3f}ms err={fmt_err(err)} "
                          f"reps={n}", flush=True)
                except Exception as e:  # noqa: BLE001
            ...
            if bomd is not None:
                try:
                    ...
                    tms, n = bench(bomd, xd)
                    rows["omd-batched"] = (
                        tms, err, n, meta[T]["ops"].get("omd"))
                    print(f"    omd-batched: {tms:.3f}ms "
                          f"err={fmt_err(err)} reps={n}", flush=True)
                    if dev == "cuda":
                        try:
                            with torch.no_grad():
                            ...
                            rows["omd-batched+graph"] = (
                                tms, err, n, None)
                            print(f"    omd-graph: {tms:.3f}ms "
                                  f"err={fmt_err(err)} reps={n}",
                                  flush=True)
                            bomd.drop_cuda_graph()
                        except Exception as e:  # noqa: BLE001
                            print(f"    omd-graph FAILED: "
                                  f"{type(e).__name__}: {e}",
                                  flush=True)
                except Exception as e:  # noqa: BLE001
                    print(f"    omd-batched FAILED: "
            ...
            if cm is not None:
                try:
                    ...
                    print(f"    inductor: {tms:.3f}ms "
                          f"err={fmt_err(err)} reps={n}", flush=True)
                except Exception as e:  # noqa: BLE001
                    print(f"    inductor bench FAILED: {e}",
                          flush=True)
            ...
            try:
                ...
                print(f"    omd-direct: {tms:.3f}ms err={fmt_err(err)} "
                      f"reps={n}", flush=True)
            except Exception as e:  # noqa: BLE001
        ...
        for (T, d), rows in all_rows.items():
            if d == dev:
                for k in rows:
                    if k not in forms:
                        forms.append(k)
        print("| T | form | ms | vs eager-IR | vs torch-eager "
              "| equiv err | ops |")
        print("|" + "---|" * 7)
        for T in sizes:
            ...
            for k in forms:
                ...
                print(f"| {T} | {k} | {tms:.3f} | {su_ir} | {su_m} "
                      f"| {fmt_err(err)} "
                      f"| {ops if ops is not None else '—'} |")
        print()

    print("\n## Build stats\n")
    print("| T | build s | enodes | omd@root | map_mode | "
          "omd tree census (top) |")
    print("|---|---|---|---|---|---|")
    for T in sizes:
        mt = meta.get(T, {})
        cen = mt.get("census_omd", {})
        cen_s = ", ".join(f"{k}:{v}" for k, v in
                          sorted(cen.items(), key=lambda kv: -kv[1])[:6])
        print(f"| {T} | {mt.get('build_s', 0):.0f} "
...
def main():
    ...
    if not args.skip_gap:
        ...
        gap_report(ScanAttnMH(T, 64, 4, 16).eval().double(),
                   x, f"ScanAttnMH T={T} (natural view+transpose heads)",
        ...
        egc, rootc, irc, srcc, infoc = gap_report(
            mc, x, f"ScanAttnChunkMH T={T} (chunk+stack head packing)",
        ...
        if infoc["omd_any"]:
            ...
            t = egc.extract_best(egc.find(rootc), flops_cost,
                                 overrides=pins)
            if t is not None:
                ...
                with torch.no_grad():
                    ref = mc(x)
                    out = mod(x)
                print(f"  nested-omd extracted term: root={t.op}, "
                      f"max|Δ| vs module = "
                      f"{(out-ref).abs().max().item():.2e}", flush=True)
        gap_report(ScanAttnSDPA(T, 64, 4, 16).eval().double(),
                   x, f"ScanAttnSDPA T={T} (fused kernel)",
