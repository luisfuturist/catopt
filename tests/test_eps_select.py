...
def _has_chained_linear(term):
    ...
    if not isinstance(term, Op):
        return False
    if (term.op == "linear" and term.args
...
def test_param_bytes_counts_eps_factors_normally():
    ...
    uname = next(n for n in _param_names(best) if n.startswith("eps_u_"))
    vname = next(n for n in _param_names(best) if n.startswith("eps_v_"))
...
def test_bounded_generous_budget_keeps_factorised():
    ...
    o = offers[0]
    cost = param_bytes_cost_for(src)
    b = eg.extract_best_bounded(root, cost, max_error=o["bound"] * 2,
                                src_term=ir.root)
...
def test_bounded_zero_budget_is_exact_only():
    ...
    ir, src, eg, root, offers = _saturated()
    cost = param_bytes_cost_for(src)
    lo = eg.extract_best_bounded(root, cost, max_error=0.0,
                                 src_term=ir.root)
...
def test_bounded_budget_between_sweeps_the_choice():
    ...
    o = offers[0]
    cost = param_bytes_cost_for(src)
    under = eg.extract_best_bounded(root, cost,
                                    max_error=o["bound"] * 0.5,
                                    src_term=ir.root)
    over = eg.extract_best_bounded(root, cost,
                                   max_error=o["bound"] * 1.5,
                                   src_term=ir.root)
...
def test_bounded_exact_graph_unaffected():
    ...
    for eps in (0.0, 1e-9, 1.0):
        b = eg.extract_best_bounded(root, param_bytes_cost,
                                    max_error=eps, src_term=t)
        assert b is not None
        cert = eg.certificate(t, b, root_eid=root)
        assert cert.exact
        assert cert.error_bound <= eps
    b0 = eg.extract_best_bounded(root, param_bytes_cost,
                                 max_error=0.0, src_term=t)
...
def test_bounded_none_budget_is_plain_extraction():
    ...
    cost = param_bytes_cost_for(src)
    assert op_repr(eg.extract_best_bounded(root, cost)) == \
        op_repr(eg.extract_best(root, cost))
...
def test_bounded_level1_is_vacuous():
    ...
    assert offers
    b = eg.extract_best_bounded(root, param_bytes_cost_for(src),
                                max_error=0.0, src_term=ir.root)
    assert b is not None
    assert _has_chained_linear(b)
