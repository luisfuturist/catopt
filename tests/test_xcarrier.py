...
def test_matmul_applyd_rows():
    ...
    P, K, d = 5, 7, 4
    E, a, b, h = _V("E", (P, K)), _V("a", (K, d)), _V("b", (K, d)), \
        _V("h", (d,))
    env = {E: _rand((P, K), 1), a: _rand((K, d), 2),
           b: _rand((K, d), 3), h: _rand((d,), 4)}
...
def test_matmul_applyd_rows_veto_feature_axis():
    ...
    P, K, d = 5, 7, 4
    E, a, b, h = _V("E", (P, d)), _V("a", (K, d)), _V("b", (K, d)), \
        _V("h", (d,))
    t0 = Op.make("matmul", E,
...
def test_matmul_applyd_vec_promotion():
    ...
    o, d = 6, 4
    W, a, b, h = _V("W", (o, d)), _V("a", (d,)), _V("b", (d,)), \
        _V("h", (d,))
    env = {W: _rand((o, d), 5), a: _rand((d,), 6),
           b: _rand((d,), 7), h: _rand((d,), 8)}
    t0 = Op.make("matmul", W,
                 Op.make("applyd", Op.make("aff_diag", a, b), h))
...
def test_matmul_apply_dense():
    """matmul(W, apply(aff(A,c),h)) = apply(aff(WA, Wc), h)."""
    o, i = 6, 4
    W, A, c, h = _V("W", (o, i)), _V("A", (i, i)), _V("c", (i,)), \
        _V("h", (i,))
    env = {W: _rand((o, i), 9), A: _rand((i, i), 10),
           c: _rand((i,), 11), h: _rand((i,), 12)}
    t0 = Op.make("matmul", W,
                 Op.make("apply", Op.make("aff", A, c), h))
    _law(XC.XC_MATMUL_APPLY, t0, env)
    t1 = Op.make(
        "apply",
        Op.make("aff", Op.make("matmul", W, A), Op.make("matmul", W, c)),
        h)
...
def test_linear_applyd_promotion():
    ...
    T, o, i = 8, 6, 4
    W, a, b, h = _V("W", (o, i)), _V("a", (T, i)), _V("b", (T, i)), \
        _V("h", (i,))
    env = {W: _rand((o, i), 13), a: _rand((T, i), 14),
           b: _rand((T, i), 15), h: _rand((i,), 16)}
    t0 = Op.make("linear",
                 Op.make("applyd", Op.make("aff_diag", a, b), h), W)
    out = _law(XC.XC_LINEAR_APPLYD, t0, env)
    # the promoted term really is a dense `apply`
    assert out.op == "apply"
    # vector-state form: a,b (i,)
    a1, b1 = _V("a", (i,)), _V("b", (i,))
    env1 = {W: env[W], a1: _rand((i,), 17), b1: _rand((i,), 18),
            h: env[h]}
    t0 = Op.make("linear",
                 Op.make("applyd", Op.make("aff_diag", a1, b1), h), W)
...
def test_linear_apply_dense():
    ...
    T, o, i = 8, 6, 4
    W, A, c, h = _V("W", (o, i)), _V("A", (T, i, i)), \
        _V("c", (T, i)), _V("h", (i,))
    env = {W: _rand((o, i), 19), A: _rand((T, i, i), 20),
           c: _rand((T, i), 21), h: _rand((i,), 22)}
    t0 = Op.make("linear",
                 Op.make("apply", Op.make("aff", A, c), h), W)
...
def test_scale_and_add_laws():
    ...
    d = 4
    a1, b1, a2, b2, h = (_V("a1", (d,)), _V("b1", (d,)),
                         _V("a2", (d,)), _V("b2", (d,)), _V("h", (d,)))
    A1, c1, A2, c2 = (_V("A1", (d, d)), _V("c1", (d,)),
                      _V("A2", (d, d)), _V("c2", (d,)))
    r = _V("r", ())
    env = {a1: _rand((d,), 30), b1: _rand((d,), 31),
           a2: _rand((d,), 32), b2: _rand((d,), 33),
           A1: _rand((d, d), 34), c1: _rand((d,), 35),
           A2: _rand((d, d), 36), c2: _rand((d,), 37),
           h: _rand((d,), 38), r: _rand((), 39)}
    ...
    t0 = Op.make("add", Op.make("applyd", f1, h),
                 Op.make("applyd", f2, h))
    _law(XC.XC_ADD_APPLYD, t0, env)
    t0 = Op.make("add", Op.make("apply", g1, h), Op.make("apply", g2, h))
...
def test_chunk_split_applyd():
    ...
    env = {a: _rand((K, d), 40), b: _rand((K, d), 41),
           h: _rand((d,), 42)}
...
def test_chunk_apply_dense():
    ...
    env = {A: _rand((T, i, i), 50), c: _rand((T, i), 51),
           h: _rand((i,), 52)}
...
def test_om_elem_affd():
    ...
    Tq, K, d = 5, 7, 4
    s, a, b, h = _V("s", (Tq, K)), _V("a", (K, d)), _V("b", (K, d)), \
        _V("h", (d,))
    env = {s: _rand((Tq, K), 60), a: _rand((K, d), 61),
           b: _rand((K, d), 62), h: _rand((d,), 63)}
...
def test_om_elem_affd_veto_bad_axes():
    ...
    Tq, K, d = 5, 7, 4
    s, a, b, h = _V("s", (Tq, d)), _V("a", (K, d)), _V("b", (K, d)), \
        _V("h", (d,))
    t0 = Op.make("om_elem", s,   # s keys = d ≠ K — wrong
                 Op.make("applyd", Op.make("aff_diag", a, b), h))
...
def test_om_elem_aff_dense():
    ...
    Tq, K, d, i = 5, 7, 4, 3
    s, A, b, h = _V("s", (Tq, K)), _V("A", (K, d, i)), \
        _V("b", (K, d)), _V("h", (i,))
    env = {s: _rand((Tq, K), 64), A: _rand((K, d, i), 65),
           b: _rand((K, d), 66), h: _rand((i,), 67)}
    t0 = Op.make("om_elem", s,
...
def test_fused_elem_composes_under_om():
    ...
    h = _V("h", (d,))
    env = {s1: _rand((Tq, K1), 70), s2: _rand((Tq, K2), 71),
           a1: _rand((K1, d), 72), b1: _rand((K1, d), 73),
           a2: _rand((K2, d), 74), b2: _rand((K2, d), 75),
           h: _rand((d,), 76)}
    lhs = Op.make(
        "om_apply",
        Op.make("om_compose",
    ...
    rhs = Op.make(
        ...
        Op.make("om_compose",
                Op.make("om_elem", s1,
                        Op.make("applyd",
                                Op.make("aff_diag", a1, b1), h)),
                Op.make("om_elem", s2,
                        Op.make("applyd",
                                Op.make("aff_diag", a2, b2), h))))
...
def test_omd_lift_single():
    ...
    s, a, b, h = _V("s", (Tq, K)), _V("a", (K, d)), _V("b", (K, d)), \
        _V("h", (d,))
    env = {s: _rand((Tq, K), 80), a: _rand((K, d), 81),
           b: _rand((K, d), 82), h: _rand((d,), 83)}
    t0 = Op.make("om_apply",
                 Op.make("om_elem", s,
                         Op.make("applyd",
                                 Op.make("aff_diag", a, b), h)))
    _law(XC.XC_OMD_LIFT, t0, env)
    _law(XC.XC_OMD_UNLIFT,
         Op.make("omd_apply", Op.make("omd_elem", s, a, b), h), env)
...
def test_omd_lift_dense():
    ...
    s, A, b, h = _V("s", (Tq, K)), _V("A", (K, d, i)), \
        _V("b", (K, d)), _V("h", (i,))
    env = {s: _rand((Tq, K), 84), A: _rand((K, d, i), 85),
           b: _rand((K, d), 86), h: _rand((i,), 87)}
    t0 = Op.make("om_apply",
                 Op.make("om_elem", s,
                         Op.make("apply", Op.make("aff", A, b), h)))
    _law(XC.XC_OMD_LIFT_DENSE, t0, env)
    _law(XC.XC_OMD_UNLIFT_DENSE,
         Op.make("omd_applym", Op.make("omd_elem", s, A, b), h), env)
...
def test_omd_pair_lift():
    ...
    a2, b2 = _V("a2", (K2, d)), _V("b2", (K2, d))
    h = _V("h", (d,))
    env = {s1: _rand((Tq, K1), 90), s2: _rand((Tq, K2), 91),
           a1: _rand((K1, d), 92), b1: _rand((K1, d), 93),
           a2: _rand((K2, d), 94), b2: _rand((K2, d), 95),
           h: _rand((d,), 96)}
    t0 = Op.make(
        "om_apply",
        Op.make("om_compose",
...
def test_omd_split():
    ...
    a1, b1 = _V("a1", (K1, d)), _V("b1", (K1, d))
    a2, b2 = _V("a2", (K2, d)), _V("b2", (K2, d))
    env = {s1: _rand((Tq, K1), 100), s2: _rand((Tq, K2), 101),
           a1: _rand((K1, d), 102), b1: _rand((K1, d), 103),
           a2: _rand((K2, d), 104), b2: _rand((K2, d), 105)}
    t0 = Op.make("omd_elem",
...
def test_gather_applyd_stack():
    ...
    h = _V("h", (d,))
    env = {**{a[i]: _rand((d,), 120 + i) for i in range(n)},
...
def test_gather_apply_stack_dense():
    ...
    h = _V("h", (i,))
    env = {**{A[k]: _rand((i, i), 150 + k) for k in range(n)},
...
def test_omd_tree_lift_pass():
    ...
    s1, s2, s3 = (_V("s1", (Tq, K1)), _V("s2", (Tq, K2)),
                  _V("s3", (Tq, K3)))
    a1, b1 = _V("a1", (K1, d)), _V("b1", (K1, d))
    a2, b2 = _V("a2", (K2, d)), _V("b2", (K2, d))
    a3, b3 = _V("a3", (K3, d)), _V("b3", (K3, d))
    h = _V("h", (d,))
    env = {s1: _rand((Tq, K1), 180), s2: _rand((Tq, K2), 181),
           s3: _rand((Tq, K3), 182),
           a1: _rand((K1, d), 183), b1: _rand((K1, d), 184),
           a2: _rand((K2, d), 185), b2: _rand((K2, d), 186),
           a3: _rand((K3, d), 187), b3: _rand((K3, d), 188),
           h: _rand((d,), 189)}
...
def test_egraph_xcarrier_fires():
    ...
    s, a, b, h = _V("s", (Tq, K)), _V("a", (K, d)), _V("b", (K, d)), \
        _V("h", (d,))
    env = {s: _rand((Tq, K), 200), a: _rand((K, d), 201),
           b: _rand((K, d), 202), h: _rand((d,), 203)}
    t0 = Op.make("om_apply",
                 Op.make("om_elem", s,
                         Op.make("applyd",
                                 Op.make("aff_diag", a, b), h)))
    ...
    assert meta._eval_allclose(meta._eval_term(t0, env),
                               meta._eval_term(t_any, env),
                               tol=1e-12)
...
def test_egraph_stack_then_omd_chain():
    ...
    s = _V("s", (Tq, n))
    h = _V("h", (d,))
    env = {**{a[i]: _rand((d,), 210 + i) for i in range(n)},
           **{b[i]: _rand((d,), 220 + i) for i in range(n)},
           s: _rand((Tq, n), 230), h: _rand((d,), 231)}
    vseq = Op.make(
        "stack",
        *(Op.make("applyd", Op.make("aff_diag", a[i], b[i]), h)
          for i in range(n)),
...
def test_om_compose_still_exact_on_affine_values():
    ...
    whole = om_apply(om_elem(torch.cat([s1, s2], -1),
                             torch.cat([a1 * h + b1, a2 * h + b2],
                                       -2)))
