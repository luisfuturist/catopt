...
from catopt.ir import IR, Op, Var, TensorType
...
def _chunked_kv_ir(B, H, Tq, d, dv, Tkv, C):
    ...
    root = Op.make("om_apply", _compose_tree([leaf(i) for i in range(n)]))
    return IR(root=root, inputs=[q, Kb, Vb],
...
def test_streaming_matches_dense_and_batched(n_blocks, dtype, tree):
    ...
    tks = [torch.randn(B, H, K, d, dtype=dtype) for _ in range(n_blocks)]
    tvs = [torch.randn(B, H, K, dv, dtype=dtype) for _ in range(n_blocks)]
...
def test_streaming_score_vars_and_om_leaf():
    ...
    ir = IR(root=Op.make("om_apply", tree),
    ...
    tss = [torch.randn(B, H, T, K, dtype=torch.float64) for _ in range(2)]
    tvs = [torch.randn(B, H, K, dv, dtype=torch.float64) for _ in range(2)]
...
def test_streaming_shared_leaf_multiplicity():
    ...
    ir = IR(root=Op.make("om_apply", tree), inputs=[s, v],
    ...
    ref = torch.softmax(torch.cat([ts, ts], -1), -1) \
        @ torch.cat([tv, tv], -2)
    assert (out - ref).abs().max().item() < 1e-12
...
def test_streaming_masked_nan_matches_dense():
    ...
    ir = IR(root=Op.make("om_apply", _compose_tree(
        [Op.make("om_elem", s, v) for s, v in zip(ss, vs)])),
        inputs=ss + vs,
        input_names={v.name for v in ss + vs}, params={})
    mod = to_streaming_om_module(ir).eval()
...
def test_incremental_step_equals_recompute():
    ...
    with torch.no_grad():
        ...
        for i in range(cut, n * C, C):
            st = om_step_qk(st, tq, tK[:, :, i:i + C],
                            tV[:, :, i:i + C])
        # also the raw-score step form
        st2 = om_step(None, tq @ tK[:, :, :cut].transpose(-2, -1),
                      tV[:, :, :cut])
        for i in range(cut, n * C, C):
            st2 = om_step(st2, tq @ tK[:, :, i:i + C].transpose(-2, -1),
                          tV[:, :, i:i + C])

    for a, b in zip(st, full):
        assert (a - b).abs().max().item() < 1e-12
    assert (om_apply_state(st2) - om_apply_state(full)) \
        .abs().max().item() < 1e-12
...
def test_incremental_growing_cache_matches_sdpa():
    ...
    T0, steps, C = 512, 8, 64
    q = torch.randn(B, H, Tq, d, device=DEV, dtype=torch.float64) \
        / math.sqrt(d)
    K = torch.randn(B, H, T0 + steps * C, d,
                    device=DEV, dtype=torch.float64)
    V = torch.randn(B, H, T0 + steps * C, dv,
                    device=DEV, dtype=torch.float64)
...
def test_empty_state_is_identity():
    ...
    st2 = _om_compose(st_none,
                      om_empty_state((B, H, Tq), dv,
                                     dtype=torch.float64))
...
def test_incremental_through_module_statics():
    ...
    ir = IR(root=Op.make("om_apply", Op.make("om_elem", s, v)),
            inputs=[s, v], input_names={"s", "v"}, params={})
    mod = to_streaming_om_module(ir)
    st = mod.step(None, torch.randn(B, H, Tq, 32,
                                    dtype=torch.float64),
...
def test_fallback_non_om_matches_serial():
    ...
    with torch.no_grad():
        ...
        assert torch.equal(mod.forward_state(x.clone()),
                           ref(x.clone()))
