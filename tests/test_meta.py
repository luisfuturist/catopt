...
def _recurrence_seed():
    ...
    seed = Op.make(
        "add",
        Op.make(
            "matmul", A2, Op.make("add", Op.make("matmul", A1, h0), u)
        ),
        x,
    )
...
def test_canonicalize_collapses_coherent_variants():
    ...
    t3 = Op.make(
        "add", Op.make("add", a, Const(0.0)), Op.make("add", b, c)
    )
...
def test_canonicalize_identity_and_involution():
    ...
    assert meta.canonicalize(
        Op.make("add", Const(0.0), Const(0.0))
    ) == Const(0.0)
...
def test_canonicalize_semantics_preserved():
    ...
    ir = IR(
        root=canon, inputs=[x, y], input_names={"x", "y"}, params={}
    )
...
def test_stratified_run_reduces_enodes_and_stays_exact():
    ...
    stats_full = eg_full.run(
        all_rules, root_full, max_iterations=14, max_nodes=400_000
    )
...
def test_synthesize_emits_valid_derived_rules():
    ...
    for d in derived:
        ...
        assert meta.pattern_metavars(d.rhs) <= meta.pattern_metavars(
            d.lhs
        )
...
def test_synthesis_rediscovers_lift_step_composite():
    ...
    for d in hits:
        ...
        if out == expected:
            found = True
    assert found, (
        f"no derived rule produced the fused form: "
...
def test_synthesis_with_lift_step_emits_composed_form():
    ...
    expected = Op.make(
        "apply",
        Op.make(
            "aff_compose", Op.make("aff", A2, x), Op.make("aff", A1, u)
        ),
        h0,
    )
    found = False
    for d in derived:
        ...
        if (
            subst is not None
            and meta.instantiate_pattern(d.rhs, subst) == expected
        ):
            found = True
    assert found
