...
from catopt.models import ParallelBlock, TransformerBlock
...
def main():
    ...
    for n_layers, T, B in [(2, 128, 8)]:
        ...
        top = dict(sorted(stats.get("rule_fires", {}).items(),
                          key=lambda kv: -kv[1])[:8])
        print(f"  rule fires (top): {top}")
        print(f"  {'eager':>14} {'inductor':>14} {'catopt':>14} "
              f"{'catopt+ind':>14}")
        print(f"  {eager_ms:>12.3f}ms {ind_ms:>12.3f}ms "
              f"{opt_ms:>12.3f}ms {opti_ms:>12.3f}ms")
        print(f"  speedup vs eager: {eager_ms/opt_ms:.2f}x | "
